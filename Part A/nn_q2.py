...
for batch_size in batch_sizes:
    ...
    for x in range(no_of_epochs):
        ...
        print('batch size {}, epoch {}'.format(str(batch_size), str(x+1)))
    ...
    # Plot the graphs
    plt.figure(1)
    plt.plot(range(no_of_epochs), training_costs, label=str(batch_size))
...
plt.title('Training Error vs Iterations for different Batch Sizes')
...
plt.xlabel('No. of Iterations')
plt.ylabel('Accuracy')
plt.title('Accuracy vs No. of Iterations for different Batch Sizes')
plt.legend(title='Batch Sizes')
plt.savefig('q2a_test_accuracy_vs_iterations.png')

print('times to update', str(times_to_update))
plt.figure(3)
plt.plot(batch_sizes, times_to_update)
plt.xticks(batch_sizes)
plt.xlabel('Batch Size')
plt.ylabel('Update time in microseconds')
plt.title('Time to update for different Batch Sizes')
plt.savefig('q2b_time_to_update_vs_batch_size.png')
