...
decay_param = theano.shared(0.0, theano.config.floatX)
learning_rate = 0.01
no_of_epochs = 1000
batch_size = 8
no_of_input_nodes = 36
no_of_hidden_layer_neurons = 15
...
decay_params_test = np.array([0.0, 1e-3, 1e-6, 1e-9, 1e-12])

for test_decay_param in decay_params_test:
    
    decay_param.set_value(test_decay_param)
    training_costs = []
    test_accuracy = []
    ...
    for x in range(no_of_epochs):
        ...
        print('decay parameter {}, epoch {}'.format(str(decay_param.get_value()), str(x+1)))
    ...
    # Plot the graphs
    if test_decay_param == 0:
        plt.figure(1)
        plt.plot(range(no_of_epochs), training_costs, label='decay param={0:.1f}'.format(test_decay_param))
        plt.figure(2)            
        plt.plot(range(no_of_epochs), test_accuracy, label='decay param={0:.1f}'.format(test_decay_param))
        # Also plot graphs to get a zoomed view
        plt.figure(3)
        plt.plot(range(970, 1001), training_costs[969:], label='decay param={0:.1f}'.format(test_decay_param))
        plt.figure(4)            
        plt.plot(range(970, 1001), test_accuracy[969:], label='decay param={0:.1f}'.format(test_decay_param))
    else:
        plt.figure(1)
        plt.plot(range(no_of_epochs), training_costs, label='log(decay param)={0:.1f}'.format(np.log10(test_decay_param)))
        plt.figure(2)        
        plt.plot(range(no_of_epochs), test_accuracy, label='log(decay param)={0:.1f}'.format(np.log10(test_decay_param)))
        # Also plot graphs to get a zoomed view
        plt.figure(3)
        plt.plot(range(970, 1001), training_costs[969:], label='log(decay param)={0:.1f}'.format(np.log10(test_decay_param)))
        plt.figure(4)        
        plt.plot(range(970, 1001), test_accuracy[969:], label='log(decay param)={0:.1f}'.format(np.log10(test_decay_param)))

plt.figure(1)
plt.xlabel('No. of Iterations')
plt.ylabel('Training Error')
plt.title('Training Error vs Iterations for different decay parameters')
plt.legend(title='Decay Parameter')
plt.savefig('q4a_training_error_vs_iterations.png')

plt.figure(2)
plt.xlabel('No. of Iterations')
plt.ylabel('Test Accuracy')
plt.title('Test Accuracy vs Iterations for different decay parameters')
plt.legend(title='Decay Parameter')
plt.savefig('q4b_test_accuracy_vs_iterations.png')
...
plt.legend(title='Decay Parameter')
plt.xticks(range(980, 1001, 5))
plt.savefig('q4b_test_accuracy_vs_iterations_zoomed.png')
